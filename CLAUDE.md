...
3. **Backfill tickers:** Run `npm run backfill:tickers` (or `node tools/backfill-tickers.mjs --dry-run` first). The script fetches SEC `company_tickers.json`, matches empty tickers by normalized issuer name, and writes the file. It **never overwrites** a non-empty ticker. If a normalized name maps to more than one SEC ticker, that name is treated as ambiguous and left empty.
